...
Remnawave по `User-Agent` решает, в каком виде отдать подписку, поэтому контейнер по умолчанию подставляет клиентский набор заголовков — `x-hwid`, `x-device-os`, `x-ver-os`, `x-device-model` и `user-agent`. Значение, заданное вами явно, всегда важнее подставляемого.

Переключатель «Заголовки по умолчанию» во вкладке «Запросы» отключает эту подстановку целиком. Когда он выключен, уходят только те заголовки, которые вы указали сами: панель убирает из списка нетронутые значения по умолчанию, а изменённые вручную оставляет. Это нужно там, где провайдер ждёт собственный набор заголовков или где чужой `User-Agent` мешает.

Заголовки отдельного профиля имеют приоритет над глобальными. Изменение переключателя, как и правка самих заголовков, запускает повторную загрузку активной подписки — запрос уходит уже другой.
