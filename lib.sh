...
# Пустой акцент означает родной цвет темы.
# Приёмник лога: /dev/stdout, /dev/stderr, none или абсолютный путь без
# пробелов и обхода каталогов. Негодное значение заменяется умолчанием.
valid_log_sink() {
...
# Читает несколько ключей за один запуск awk и печатает значения по строке на
# ключ в порядке перечисления. Значение отделяется по первому '=', поэтому
# base64 не теряет padding. Замыкающая точка защищает последнее пустое
# значение от срезания подстановкой команд.
state_load() {
  state_load_file=$1
  shift
...
event_log() {
  ...
  # Дублируем в stdout контейнера — RouterOS показывает его в /log. Кроме CGI:
  # там stdout это тело HTTP-ответа.
...
# Сравнивает кандидата с работающим runtime: совпал — Xray не перезапускается.
# Сравниваются фрагменты confdir, отпечаток исходника, выбранная позиция и
# ключ набора geodata. Содержимое файлов geodata не сравнивается: их обновляет
# сам Xray, и это выглядело бы как изменение конфигурации.
...
# state_load отдаёт пустую строку и для отсутствующего ключа, и для пустого;
# валидация ниже приводит каждое поле к допустимому диапазону.
load_settings() {
...
EOF
  ...
  # Пустое значение убирает ключ из конфигурации.
  ...
  # В Xray `mixed` — алиас SocksServerConfig, поэтому пишем протокол `socks`.
...
build_xray_config() {
  ...
  # dokodemo-door и tun из подписки вырезаются всегда, socks/mixed и http — по
  # флажкам.
  ...
  # фрагмент подписки, чтобы ссылки на geosite: из оверрайда учитывались ниже.
  ...
  # Секция geodata убирается из фрагмента подписки, когда на геобазы никто не
  # ссылается: оверлей идёт позже и её собственную секцию не отменяет.
...
# Хеш генерирует тот же busybox, что его и проверяет. md5crypt — запасной
# вариант, когда sha512 недоступен. Печатает две строки: хеш и алгоритм.
password_hash() {
  password_hash_value=
